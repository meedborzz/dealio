...
import { TrendingUp, Users, Building2, Star, Calendar, BarChart3, ArrowUp, ArrowDown, Target, CheckCircle, Clock, XCircle } from 'lucide-react';
...
const AdminStatsPage: React.FC = () => {
  ...
  const fetchAdvancedStats = async () => {
    try {
      ...
      const [totalBusinessesResult, totalClientsResult, totalBookingsResult] = await Promise.all([
      ...
      const businessesWithRating = businesses.filter(b => b.rating && b.rating > 0);
      const averageRating = businessesWithRating.length > 0
        ? businessesWithRating.reduce((sum, b) => sum + (b.rating || 0), 0) / businessesWithRating.length
        : null;
  ...
  const getGrowthColor = (growth: number) => {
    if (growth >= 20) return 'from-green-500 to-emerald-600';
    if (growth >= 10) return 'from-yellow-500 to-teal-500';
    if (growth >= 0) return 'from-teal-500 to-amber-600';
